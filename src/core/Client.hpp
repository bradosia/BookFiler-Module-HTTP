...
private:
  ...
  std::shared_ptr<std::map<std::string, std::string>> headersMapPtr;
...
public:
  ClientImpl();
  ClientImpl(std::map<std::string, newClientVariantType> map);
  ~ClientImpl();
  ...
  int setCookie(std::map<std::string, std::string>);
