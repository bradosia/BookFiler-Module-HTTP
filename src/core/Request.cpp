...
std::shared_ptr<Cookie> RequestImpl::getCookie(const std::string name_){
    std::shared_ptr<CookieImpl> cookie = std::make_shared<CookieImpl>();
    return std::dynamic_pointer_cast<Cookie>(cookie);
}

std::string RequestImpl::getUserAgentBrowser(){
