...
/*  Load a signed certificate into the ssl context, and configure
    ...
    accompanying the Beast certificate for more details.
*/
inline void loadServerCertificate(std::shared_ptr<boost::asio::ssl::context> sslContext) {
  /*
      The certificate was generated from CMD.EXE on Windows 10 using:
      winpty openssl dhparam -out dh.pem 2048
      winpty openssl req -newkey rsa:2048 -nodes -keyout key.pem -x509 -days
  ...
  */
  ...
  sslContext->use_certificate_chain(boost::asio::buffer(cert.data(), cert.size()));
